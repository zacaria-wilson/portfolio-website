    ...
    function contstructSliderClass() {
        ...
        class Slider {
            ...
            async windowResize(){
                ...
                if(this.windowW >= 991){
                    try{
                        ...
                        console.error(this.name, 'An error occurred during windowResize, where windowW >= 991', error);
                    }
                    //Un-folds folded descriptions.
            ...
            async responsiveInit(){
                ...
                if (this.windowW <= 991){
                    check('Mobile Mode Check: window width is', this.windowW);
            ...
            */
            ...
            async sliderSync(t) {
                ...
                let parentActiveIndex;
                let childActiveIndex;

                
                check(this.name, 'sliderSync. Parent is:', {parent}, 'and Child is:', {child})
                check(this.name,'sliderSync: this.activeSlide is', this.activeSlide)

                if (parent && parent.activeSlide){
                    parentActiveIndex = parent.activeSlide.getAttribute(parent.slideIndexAttribute)
                    check(this.name, 'has Parent and parent active slide', parentActiveIndex)
                    if(parentActiveIndex != t){
                        check(this.name, 'tells parent to scrollTo;', t)
                        parent.scrollTo(t)}
                } else if (parent && !parent.activeSlide){
                    check(this.name, 'Parent but no active slide')
                    parent.updateSlider(t)
                };

                if (child && child.activeSlide){
                    childActiveIndex = child.activeSlide.getAttribute(child.slideIndexAttribute)
                    check(this.name, 'has Child and child active slide', childActiveIndex)
                    childActiveIndex = child.activeSlide.getAttribute(child.slideIndexAttribute)
                    if(childActiveIndex != t){
                        check(this.name, 'tells child to scrollTo;', t)
                        child.scrollTo(t)}
                } else if (child && !child.activeSlide){
                    check(this.name, 'Child but no active slide', child.activeSlide)
                    child.updateSlider(t)
                };
                */
